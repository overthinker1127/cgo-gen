...
```bash
cargo run --bin cgo-gen -- check --config examples/11-overloading/config.yaml
cargo run --bin cgo-gen -- generate --config examples/11-overloading/config.yaml --dump-ir
```
