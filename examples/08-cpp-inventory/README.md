...
```bash
cargo run --bin cgo-gen -- check --config examples/08-cpp-inventory/config.yaml
cargo run --bin cgo-gen -- generate --config examples/08-cpp-inventory/config.yaml --dump-ir
```
