...
```bash
cargo run --bin cgo-gen -- check --config examples/10-owner-return/config.yaml
cargo run --bin cgo-gen -- generate --config examples/10-owner-return/config.yaml --dump-ir
```
