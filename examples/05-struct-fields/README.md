...
```bash
cargo run --bin cgo-gen -- check --config examples/05-struct-fields/config.yaml
cargo run --bin cgo-gen -- generate --config examples/05-struct-fields/config.yaml --dump-ir
```
